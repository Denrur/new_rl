...
#
# Also used for obstructions; for the purposes of obstructions,
# the center variable is ignored.
...
class FOVCalc(object):
    ...
    # Changing the radius-fudge changes how smooth the
    # edges of the vision bubble are.
    #
    ...
    #
    # For example, let's say you you have obstructions blocking
    # (0.0 - 0.25) and (.33 - 1.0).
    ...
    # visible, but the blocks in front of it will
    ...
    #
    # 0 - if you have a line to the near, center, or far,
    # it will return as visible
    # 1 - if you have a line to the center and at least one
    # other corner it will return as visible
    # 2 - if you have a line to all the near, center,
    # and far, it will return as visible
    #
    # If any other value is given, it will treat it as a 2.
    RESTRICTIVENESS = 1

    # If VISIBLE_ON_EQUAL is False, an obstruction will obstruct
    # its endpoints. If True, it will not.
    #
    # For example, if there is an obstruction (0.0 - 0.25)
    # and a square at (0.25 - 0.5), the square's near angle will
    ...
    def calc_visible_cells_from(self, x_center, y_center, radius,
                                func_transparent):
        cells = self._visible_cells_in_quadrant_from(x_center, y_center,
                                                     1, 1, radius,
                                                     func_transparent)
        cells.update(self._visible_cells_in_quadrant_from(x_center, y_center,
                                                          1, -1, radius,
                                                          func_transparent))
        cells.update(self._visible_cells_in_quadrant_from(x_center, y_center,
                                                          -1, -1, radius,
    ...
    # Parameters quad_x, quad_y should only be 1 or -1.
    # The combination of the two determines the quadrant.
    # Returns a set of (x, y) tuples.
    def _visible_cells_in_quadrant_from(self, x_center, y_center,
                                        quad_x, quad_y, radius,
                                        func_transparent):
        cells = self._visible_cells_in_octant_from(x_center, y_center,
                                                   quad_x, quad_y, radius,
                                                   func_transparent, True)
        cells.update(self._visible_cells_in_octant_from(x_center, y_center,
                                                        quad_x, quad_y, radius,
                                                        func_transparent,
                                                        False))
        return cells
    ...
    def _visible_cells_in_octant_from(self, x_center, y_center,
                                      quad_x, quad_y, radius,
                                      func_transparent, is_vertical):
        iteration = 1
        visible_cells = set()
        obstructions = list()
        ...
        # End conditions:
        #   iteration > radius
        #   Full obstruction coverage (indicated by one object in the
        # obstruction list covering the full angle from 0
        #      to 1)
        while iteration <= radius and not (len(obstructions) == 1 and
            ...
            angle_allocation = 1 / num_cells_in_row

            # Start at the center (vertical or horizontal line)
            # and step outwards
            for step in range(iteration + 1):
                cell = self._cell_at(x_center, y_center,
                                     quad_x, quad_y,
                                     step, iteration, is_vertical)

                if self._cell_in_radius(x_center, y_center, cell, radius):
                    cell_angles = CellAngles(near=(step *
                                                   angle_allocation),
                                             center=((step + .5) *
                                                     angle_allocation),
                                             far=((step + 1) *
                    ...
                    if self._cell_is_visible(cell_angles, obstructions):
                        ...
                        if not func_transparent(*cell):
                            obstructions = self._add_obstruction(obstructions,
                                                                 cell_angles)
    ...
    # Returns a (x, y) tuple.
    def _cell_at(self, x_center, y_center, quad_x, quad_y,
                 step, iteration, is_vertical):
        if is_vertical:
    ...
    def _cell_in_radius(self, x_center, y_center, cell, radius):
        ...
        return cell_distance <= radius + self.RADIUS_FUDGE
    ...
    def _cell_is_visible(self, cell_angles, obstructions):
        ...
        if self.RESTRICTIVENESS == 0:
            ...
            return (center_visible and near_visible) or (center_visible and
                                                         far_visible)
        else:
    ...
    # (removing them if they are combined) and
    # adding the resulting obstruction to the list.
    #
    # Returns the generated list.
    def _add_obstruction(self, obstructions, new_obstruction):
        new_object = CellAngles(new_obstruction.near,
                                new_obstruction.center,
                                new_obstruction.far)
        new_list = [o for o in obstructions if
                    not self._combine_obstructions(o, new_object)]
...
if __name__ == '__main__':
    ...
    map_size = 20
    center_x = int(map_size / 2)
    center_y = int(map_size / 2)
    radius = 10

    # False values are obstructed; True are unobstructed.
    map = [
        [(lambda: prob_is_obstructed < random.uniform(
            0, 1))() for _ in range(map_size)] for _ in range(map_size)]
    ...
    def is_unobstructed(x, y):
        try:
            ...
            return False
    fov = FOVCalc()
    print(timeit.timeit(fov.calc_visible_cells_from(center_x, center_y,
                                                    radius, is_unobstructed)))
